...
// ✅ Handle form submission (Add new applicant)
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    // ✅ Insert new record into applicant_profile
    $stmt = $mysqli->prepare("
        INSERT INTO applicant_profile 
        (full_name, date_of_birth, gender, contact_no, email, current_address, education, skills, work_experience, status)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'Active')
    ");
    ...
    if ($stmt) {
        $stmt->bind_param(
            "sssssssss",
            $full_name, $birth_date, $gender, $contact_no, $email,
        ...
        header("Location: ../appl_list.php?added=1");
        exit();
    } else {
...
<section id="edit-profile" class="edit-profile-container">
    <div class="profile-modal">
        ...
        <form id="profile-form" class="profile-form" method="POST" action="forms/add_appl.php" enctype="multipart/form-data">
